...
// GET /api/products
// Query params: limit, featured, category, search
...
// GET /api/products/search (place BEFORE :id to avoid route shadowing)
router.get('/search', async (req, res) => {
  try {
    ...
    const maxPrice = parseInt(req.query.maxPrice || '100000000', 10);
    const sortBy = (req.query.sortBy || 'newest').toString();
    const offset = (page - 1) * limit;

    // Build WHERE conditions
    let whereConditions = ['price BETWEEN ? AND ?'];
    ...
    const orderSql = sortBy === 'price-low' ? 'price ASC'
    ...
    const [products] = await pool.execute(
      `SELECT * FROM products ${whereClause} ORDER BY ${orderSql} LIMIT ? OFFSET ?`,
      [...params, limit, offset]
    );
...
// Test endpoint to debug MySQL issues (must be after /:id route)
